...
**`specs/` is spec-first work only — never retroactive docs, never an ad-hoc audit.** Two things commonly get misfiled here: (1) documentation of code that already exists, written after the fact with no ticket ever built from it — that belongs in `docs/system/` via `/sdd-pipeline:docs`, its own separate tree with no spine ID; (2) a one-off cross-feature consistency/audit report — that belongs in `docs/sdd/reports/{date}-{slug}.md` with a `Status: OPEN|RESOLVED` header, never a borrowed feature number. Both misfilings look harmless individually but corrupt the spine's meaning for real spec-first work in the same project.
