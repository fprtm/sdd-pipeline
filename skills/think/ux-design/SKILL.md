...
**Record which grounding was actually used — this is not optional.** `design.md` must carry a `**Design source**:` line stating one of: `ui-ux-pro-max` (external plugin), a specific `docs/design-system-styles/<slug>.md` filename, `respect-existing` (an existing design was found and followed per §0.1), or `custom: <one-sentence reason>` (grounding was deliberately skipped or improvised — say why). This is the same principle as `sdlc-reason`: a decision without a recorded reason is a decision nobody can audit later, and "the direction sounds thought-through" is not evidence it was actually grounded in a real reference rather than invented in the moment. `check-file-hygiene.mjs` enforces this line is present.
