...
3. **Hand-off to plan**: if the user follows up with "build this" / "let's do it", the shared understanding from the grill session feeds directly into `docs/sdd/plans/current.md`. THINK-phase questions already settled in the grill are NOT re-asked by elicitation.
