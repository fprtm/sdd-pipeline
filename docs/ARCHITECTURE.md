...
| **ticket-decomposition** | Splits `large` tasks into vertical slices (never layer-splits) with computed blocking edges; **right-sizing check** against padding (ticket that only wires an adjacent one) before finalizing; expand→migrate→contract exception for wide mechanical refactors; T1/T2/T3 tiers; optional GitHub Issues mirror (asked, never assumed); Kanban ticket-status flow; **`00-index.md`** is the feature's formalized entry point — spec/ERD/ADR refs + a "How to Review This Feature" reading-order section + status table + frontier | `docs/sdd/traceability.md` (global TICKET-xxx), `check-parallel-safety.mjs`, `build/git-workflow` | `docs/sdd/specs/{NNN}-{slug}/tickets/{NN}-{ticket-slug}.md` + `00-index.md` |
