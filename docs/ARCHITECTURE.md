...
**A stale claim worth flagging**: `install/install.sh` and `docs/INSTALL.md` still say *"Cursor has limited skill support, so only the orchestrator is copied... use Claude Code or Codex for full SDD Pipeline"* and have the Cursor target copy just `orchestrator/SKILL.md` into `.cursor/rules/`. That was accurate before Cursor's January 2026 Agent Skills release; it no longer is — Cursor now supports the identical discovery convention (including the same `.agents/skills/` compat path Codex already installs into). The installer's actual Cursor *behavior* hasn't been changed here — only the surrounding docs/comments were corrected to stop overclaiming Cursor's limitation. Revisiting whether `--agent cursor` should install the full skill tree into a properly-nested `.agents/skills/sdd-<skill-name>/SKILL.md`-style structure (one directory per skill, matching what the discovery spec expects) is a separate, untested change — flagged here, not made.
